...
void loop() 
{
  ...
  // Serial.print(heading_demand);
  // Serial.print(", ");
  // Serial.print(pose.getTheta());
...
void turnToTarget() {
  if (!heading_target_found) {
    heading_demand = pose.getHeadingTarget();
    heading_target_found = true;
  }
  
  updateHeadingPID(0.0);
  if (heading_demand - pose.getTheta() < 2.0 ) {
    STATE = STATE_STOP;
  }
}
