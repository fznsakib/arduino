...
volatile long last_count_left_e;
volatile long last_count_right_e;
...
volatile long left_e_speed;
volatile long right_e_speed;

long left_e_prev_speed;
long right_e_prev_speed;
...
ISR( INT6_vect ) {
  ...
  // Speed is 0 if count has not changed
  if (count_right_e == last_count_right_e) {
    right_e_speed = 0.0;
  } else {
    // Calculate speed in count/s
    right_e_interval = micros() - right_e_last_time;
    float s_interval = (float)right_e_interval/1000000;

    float count_diff = count_right_e - last_count_right_e;

    right_e_speed = count_diff/s_interval;

    // Update variables
    right_e_last_time = micros();
    last_count_right_e = count_right_e;
  }
...
ISR( PCINT0_vect ) {
  ...
  // Speed is 0 if count has not changed
  if (count_left_e == last_count_left_e) {
    left_e_speed = 0.0;
  } else {
    // Calculate speed in count/s
    left_e_interval = micros() - left_e_last_time;
    float s_interval = (float)left_e_interval/1000000;

    float count_diff = count_left_e - last_count_left_e;

    left_e_speed = count_diff/s_interval;

    // Update variables
    left_e_last_time = micros();
    last_count_left_e = count_left_e;
  }
}
