...
void setup() {
  ...
  count_to_travel = cm_count * dist_to_travel; 
  count_to_rotate = (cm_count * circumference) * (angle_to_rotate/360); 
...
void loop() {
  ...
  elapsed_time = micros() - ts;
  change_in_count = count_right_e - prev_encoder_count;

  Serial.print("Encoder count change: ");
  ...
  current_speed = (float)change_in_count/(float)elapsed_time;

  // Update timestamp and count  
  ts = micros();
  prev_encoder_count = count_right_e;
...
bool rotation_complete() {
  if (abs(count_left_e) > count_to_rotate && abs(count_left_e) > count_to_rotate) {
    return true;
  }
  return false;
}

bool moving_complete() {
  if (abs(count_left_e) > count_to_travel && abs(count_right_e) > count_to_travel) {
    return true;
  }
  return false;
}
